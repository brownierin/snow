...
function run_semgrep {
  ...
  repos=`cat $WORKSPACE/snow/languages/php/enabled`

  for repo in $repos; do
    echo on this repo: $repo
    outfile="results-php-$repo.json"
		...
		docker run --rm -v "${WORKSPACE}/snow:/src" \
			...
			--config=/src/languages/php/ --json -o /src/$results/$outfile --error repositories/$repo
		code=$?
		exit_codes+=$code
		exit_codes+=' '
	done
